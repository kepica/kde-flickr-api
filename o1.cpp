...
void O1::link() {
    qDebug() << "O1::link procedura START";
    if (linked()) {
...
void O1::onTokenRequestFinished() {
    qDebug() << "O1::onTokenRequestFinished da te pitam ... ";
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    reply->deleteLater();
    if (reply->error() != QNetworkReply::NoError) {
        qWarning() << "O1::onTokenRequestFinished sa greskom : " << reply->errorString();
        return;
    }
    ...
    qDebug() << "O1:token " << requestToken_;
    qDebug() << "O1:secret " << requestTokenSecret_;
...
// #else
    ...
    url.setQuery(query);
// #endif
    qDebug() << "O1: mozda otvorim browser !";
...
void O1::onTokenExchangeFinished() {
    qDebug() << "O1::onTokenExchangeFinished smo cekali ! ";

    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    reply->deleteLater();
    if (reply->error() != QNetworkReply::NoError) {
        qWarning() << "O1::onTokenExchangeFinished sa greskom : " << reply->errorString();
        return;
    }
    ...
    if (response.contains(O2_OAUTH_TOKEN) && response.contains(O2_OAUTH_TOKEN_SECRET)) {
        QString pravi_token = response.take(O2_OAUTH_TOKEN);
        QString pravi_secret = response.take(O2_OAUTH_TOKEN_SECRET); 
        setToken(pravi_token);
        setTokenSecret(pravi_secret);
        qDebug() << "O1: pravi Token " << pravi_token;
        qDebug() << "O1: pravi Secret " << pravi_secret;
