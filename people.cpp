...
People::People(QWidget *parent, QSettings *mset) : QDialog(parent)
{
    ...
    lin_id->setText(mes3) ;
    // lin_id->setInputMask("000000000A99");
    
    m_win->user_id = mes3;
    
    QCompleter *completer = new QCompleter(id_list, this);
    completer->setCaseSensitivity(Qt::CaseInsensitive);
    lin_id->setCompleter(completer);
    ...
    connect(btn8, SIGNAL(clicked()), this, SLOT( get_people() ) );    // start search
...
void People::set_id()
{
    QString m_id = lin_id->text();    
    m_win->user_id = m_id;
    p_set->setValue("last_user_id", m_id);
    qDebug() << m_id;
}
